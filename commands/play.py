...
async def play(game, bot, pseudo, channel, msg):
    ...
    if success:
        ...
        if game.current_card.split("_")[1] == "undefined":
            ...
            await bot.send(
                f"PRIVMSG {channel} :\x02La nouvelle couleur est {color}. C'est à {game.current_player.pseudo} de jouer.\x02"
            )
        else:
            current_card = (
            ...
            await bot.send(
                f"PRIVMSG {channel} :\x02La nouvelle carte est {current_card}. C'est à {game.current_player.pseudo} de jouer.\x02"
            )
        ...
        match message:
            case "NOT_STARTED":
                await bot.send(
                    f"PRIVMSG {channel} :\x02La partie n'a pas encore commencé.\x02"
                )
            case "NOT_YOUR_TURN":
                await bot.send(
                    f"PRIVMSG {channel} :\x02Ce n'est pas à ton tour de jouer.\x02"
                )
            case "NO_CARD":
                await bot.send(
                    f"PRIVMSG {channel} :\x02Tu n'as choisit aucune carte.\x02"
                )
            case "NOT_IN_HAND":
                await bot.send(
                    f"PRIVMSG {channel} :\x02Cette carte n'est pas dans ta main.\x02"
                )
            case "INVALID":
                await bot.send(f"PRIVMSG {channel} :\x02Ce coup est invalide.\x02")
            case "NO_COLOR":
                await bot.send(
                    f"PRIVMSG {channel} :\x02Il faut donner une couleur lorsque l'on utilise une carte joker (rouge, vert, bleu ou jaune).\x02"
                )
            case "END":
                winners = []
                if len(game.finish_order) == 0:
                    await bot.send(
                        f"PRIVMSG {channel} :\x02La partie est terminée, personne n'a gagné.\x02"
                    )
                else:
