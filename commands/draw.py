...
async def draw(game, pseudo):
    ...
    if success:
        match message:
            case "OK":
                await game.bot.send(f"PRIVMSG {game.channel} :\x02{game.current_player.pseudo} a pioché une carte ! Mon petit doigt me dit que la situation va se débloquer...\x02")
            case "PASS":
                await game.bot.send(f"PRIVMSG {game.channel} :\x02{game.current_player.pseudo} a pioché une carte ! Malheureusement je crois que ça ne va pas suffire, {game.current_player.pseudo} va devoir passer son tour.\x02")
                ...
                if game.current_card.split('_')[1] == 'undefined':
                    ...
                    await game.bot.send(f"PRIVMSG {game.channel} :\x02C'est à {game.current_player.pseudo} de jouer. La couleur est {color}.\x02")
                else:
                    current_card = COLORS[game.current_card.split(
                        '_')[0]] + ' ' + game.current_card

                    await game.bot.send(f"PRIVMSG {game.channel} :\x02 C'est à {game.current_player.pseudo} de jouer. La carte est {current_card}.\x02")
        ...
        await game.bot.send(f"NOTICE {game.current_player.pseudo} :\x02Tu as pioché la carte {drawed_card}.\x02")
    else:
        match message:
            case "NOT_STARTED":
            ...
            case "ALREADY_DRAW":
                await game.bot.send(f"PRIVMSG {game.channel} :\x02Tu as déjà pioché.\x02")
            case "MOVE_POSSIBLE":
                await game.bot.send(f"PRIVMSG {game.channel} :\x02Tu peux déjà jouer sans piocher de nouvelle carte.\x02")
